...
class Like {
  constructor() {
    if (document.querySelector('.like-box')) {
      (axios__WEBPACK_IMPORTED_MODULE_0___default().defaults).headers.common['X-WP-Nonce'] = uniData.nonce;
      this.events();
    }
  }
  events() {
  ...
  async createLike(currentLikeBox) {
    try {
      const response = await axios__WEBPACK_IMPORTED_MODULE_0___default().post(uniData.root_url + '/wp-json/university/v1/manageLike', {
...
class MyNotes {
  ...
  async deleteNote(e) {
    const thisNote = this.findNearestParentLi(e.target);
    try {
      const response = await axios__WEBPACK_IMPORTED_MODULE_0___default()["delete"](uniData.root_url + '/wp-json/wp/v2/note/' + thisNote.getAttribute('data-id'));
  ...
  async updateNote(e) {
    ...
    try {
      const response = await axios__WEBPACK_IMPORTED_MODULE_0___default().post(uniData.root_url + '/wp-json/wp/v2/note/' + thisNote.getAttribute('data-id'), ourUpdatedPost);
      this.makeNoteReadOnly(thisNote);
    } catch (e) {
      console.log('Sorry');
    }
  ...
  async createNote() {
    ...
    try {
      const response = await axios__WEBPACK_IMPORTED_MODULE_0___default().post(uniData.root_url + '/wp-json/wp/v2/note/', ourNewPost);
...
class Search {
  ...
  getJson(type = 'posts') {
    return new Promise((resolve, reject) => {
      fetch(uniData.root_url + `/wp-json/uni/v1/search?term=` + this.searchField.value).then(response => response.json()).then(data => {
        resolve(data);
      }).catch(() => {
        reject();
      });
